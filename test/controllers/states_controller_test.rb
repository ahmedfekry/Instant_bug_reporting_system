...
class StatesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create state" do
    assert_difference('State.count') do
      post states_url, params: { state: { device: @state.device, memory: @state.memory, os: @state.os, storage: @state.storage } }
    end
  ...
  test "should update state" do
    patch state_url(@state), params: { state: { device: @state.device, memory: @state.memory, os: @state.os, storage: @state.storage } }
